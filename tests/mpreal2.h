...
#ifndef __MPREAL2_H__
#define __MPREAL2_H__
...
public:
    ...
    inline static mp_rnd_t & _get_default_rnd()
    {
        static mp_rnd_t rnd = MPFR_RNDN;
        return rnd;
    }
    inline static mp_prec_t &_get_default_prec()
    {
        static mp_prec_t prec = 53;
        return prec;
    }

    // Get default rounding mode & precision
    inline static mp_rnd_t   get_default_rnd()    {    
        return _get_default_rnd();
    }
    inline static mp_prec_t  get_default_prec()
    {
        return (_get_default_prec)();
    }
...
namespace std
{
    inline const mpfr2::mpreal& min(const mpfr2::mpreal& a, const mpfr2::mpreal& b, bool omitnan = false)
    {
        if(omitnan)
        {
                 if(isnan(a)) return b;
    ...
    inline const mpfr2::mpreal& max(const mpfr2::mpreal& a, const mpfr2::mpreal& b, bool omitnan = false)
    {
        if(omitnan)
        {
                 if(isnan(a)) return b;
    ...
    template <>
    inline void swap(mpfr2::mpreal& x, mpfr2::mpreal& y)
    {
        return mpfr2::swap(x, y);
    }

    template<>
    class numeric_limits<mpfr2::mpreal>
    {
    public:
